import { getBrightness } from "../brightness/brightness";
import { getTrueLightness } from "../get/get";
import { minmax } from "../helpers/helpers";
import { isRGB, isHex, isHSL, isRGBA, isHSLA } from "../is/is";
import { toHSL, toRGB, toType } from "../to/to";
...
export const textColor = (
  ...
  const opts = {
    ...
    light: '#ffffff',
    offset: 50,
    return: 'name' as 'color' | 'name',
    ...args,
  };
  const isLightBg = getTrueLightness(value) > opts.offset;
  if (opts.return === 'name') {
    return isLightBg ? 'dark' : 'light';
  }
  return isLightBg ? opts.dark : opts.light;
};
